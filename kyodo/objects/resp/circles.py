...
class CircleListingTask:
	def __init__(self, data: dict):
		...
		self.id: str = data.get("id")
		self.goal: int = data.get("goal")
		self.current: int = data.get("current")


class CircleListingTasks:
	def __init__(self, data: dict):
		data = data or {}
		self.data=data

		self.listingAvailable: bool = data.get("listingAvailable")
		self.taskCompletedCount: int = data.get("taskCompletedCount")

		self.taskList: list[CircleListingTask] = [CircleListingTask(x) for x in data.get("taskList", [])]
...
class CircleReportList:
	def __init__(self, data: dict):
		data = data or {}
		self.data=data
		self.reportList: list[ReportInfo] = [ReportInfo(x) for x in data.get("reportList", [])]
		self.pagination: dict = data.get("pagination")